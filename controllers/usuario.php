 ...
 *
 * Encargada del registro y validacion de los usuarios al momento de la votacion.
 * @package votaciones
...
{
	/**
	 * Obtiene el ID de la sesion PHP para transformarla en una sesion CodeIgniter y posteriormente guardar los datos a traves del modelo Usuario
	 * @access public
	 */
	function upasaporte()
	{
		...
		if(strlen($sid) == 0)
		{
			...
			$this->session->set_flashdata('errores', array("No se ha encontrado la sesi&oacute;n U-Pasaporte."));
			redirect('/', 'location');
		}
		else
		{
			...
			{
				$this->load->helper('url');
				$this->session->set_flashdata('errores', array("No se han encontrado los datos de usuario."));
				redirect('/', 'location');
			}
			else
			{
				...
				if( ! $registrado)
				{
					$this->session->set_flashdata('errores', array("No se ha podido registrar el usuario. Ingrese nuevamente."));
					redirect('/', 'location');	
				}
				else
				{
