 ...
 *
 * Esta clase se encarga de recibir las consultas y obtener datos a traves de los modelos para finalmente desplegarlos mediante las vistas.
 * @package votaciones
...
{
	/**
	 * Redirecciona a la lista de votos si el usuario ha ingresado correctamente.
	 * @access public
	 */
	function index()
	{
	...
	/**
	 * Obtiene los datos de los votos disponibles a traves del modelo Voto y muestra la lista de ellos a traves de la vista votos_lista.
	 * @access public
	 */
 	function lista()
	{
	...
	/**
	 * Recibe el HTTP POST a traves de CodeIgniter y guarda los datos con el modelo Voto. Posteriormente despliega el resultado en la vista votos_emitir.
	 * @access public
	 */
	function emitir()
	{
		...
		{
			$this->session->set_flashdata('errores', array("Debe indicar el n&uacute;mero de voto"));
			redirect('/votos/lista', 'location');
		}
		else if( ! $this->session->userdata('sufragio_hash') || time() - $this->session->userdata('sufragio_hash_time') > 600)
		{
			$this->session->set_flashdata('errores', array("Acceso denegado. Debe Ingresar v&iacute;a U-Pasaporte."));
